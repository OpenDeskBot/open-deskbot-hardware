...
bool WifiClient::load_wifi_from_file() {
  ...
  log_info("WiFi credentials loaded from %s", WIFI_CONF_FILE);
  return true;
}

bool WifiClient::load_wifi_defaults() {
  if (!wifi_defaults_configured()) {
    return false;
  }
  ssid = WIFI_DEFAULT_SSID;
  password = WIFI_DEFAULT_PASSWORD;
  log_info("No saved WiFi; trying defaults from wifi_defaults.h (ssid=%s)", ssid.c_str());
  return true;
}

void WifiClient::setup_wifi() {
  int connection_attempts = 0;
  bool using_saved_file = false;

  oled_print("\nConnect WiFi");
  while (WiFi.status() != WL_CONNECTED) {
    using_saved_file = false;
    if (load_wifi_from_file()) {
    ...
    if (connection_attempts > MAX_RECONNECT_ATTEMPTS) {
      if (using_saved_file) {
        ...
        log_warn("Default WiFi failed, enter AP config mode");
        WiFi.disconnect();
        connection_attempts = 0;
        ssid = "";
        password = "";
...
void WifiClient::send_ip() {
  send_broadcast_msg(String(UDP_DISCOVERY_PREFIX) + "_" + local_ip);
}
