...
void executeFactoryCommand(String cmd) {
  if (cmd == "reboot" || cmd == "restart") {
    ...
    // head_move_abs_ex <x_deg> <y_deg> <step_deg> [hold_ms] [tick_ms]
    int v[5] = {0};
    int n = 0;
    int i = cmd.indexOf(' ');
    while (n < 5 && i >= 0) {
    ...
    if (n < 3) {
      log_warn("[Factory] head_move_abs_ex x y step [hold_ms] [tick_ms]");
    } else {
      int x = v[0];
      int y = v[1];
      int step = v[2];
      uint16_t hold_ms = (n >= 4 && v[3] > 0) ? (uint16_t)v[3] : 0;
      uint16_t tick_ms = (n >= 5 && v[4] > 0) ? (uint16_t)v[4] : 0;
      if (step < 0) step = 0;
      if (step > 255) step = 255;
      head_move_abs_ex(x, y, (uint8_t)step, hold_ms, tick_ms);
      log_info("[Factory] head_move_abs_ex %d %d step=%d hold=%u tick=%u",
               x, y, step, (unsigned)hold_ms, (unsigned)tick_ms);
    }
    ...
    // head_move_ex <dx> <dy> <step_deg> [hold_ms] [tick_ms]
    int v[5] = {0};
    int n = 0;
    int i = cmd.indexOf(' ');
    while (n < 5 && i >= 0) {
    ...
    if (n < 3) {
      log_warn("[Factory] head_move_ex dx dy step [hold_ms] [tick_ms]");
    } else {
      int dx = v[0];
      int dy = v[1];
      int step = v[2];
      ...
      if (step < 0) step = 0;
      if (step > 255) step = 255;
      head_move_ex(dx, dy, (uint8_t)step, hold_ms, tick_ms);
      log_info("[Factory] head_move_ex %d %d step=%d hold=%u tick=%u",
               dx, dy, step, (unsigned)hold_ms, (unsigned)tick_ms);
    }
