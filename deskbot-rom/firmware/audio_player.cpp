...
/* audio_yield_hook 默认空实现，asr_chat_client.cpp 提供强定义覆盖以推 face tracking。
...
static void play(const int16_t* data, size_t length, float volume_ratio) {
  size_t bytes_written;
  constexpr size_t kYieldStride = 128;
  for (size_t i = 0; i < length; i++) {
    int16_t scaled_sample = static_cast<int16_t>(static_cast<float>(data[i]) * volume_ratio);
    i2s_write(I2S_NUM_1, &scaled_sample, sizeof(int16_t), &bytes_written, portMAX_DELAY);
    if ((i & (kYieldStride - 1)) == 0) {
      audio_yield_hook();
    }
  }
}
...
static bool audio_play_wav_impl(const uint8_t* data, size_t len, float volume_ratio) {
  ...
  for (size_t i = 0; i < tail_samples; ++i) {
    i2s_write(I2S_NUM_1, &zero, sizeof(int16_t), &bytes_written, portMAX_DELAY);
    if ((i & 0x7Fu) == 0) {
      audio_yield_hook();
    }
  }
...
static bool audio_play_pcm16_impl(const int16_t* pcm, size_t samples, float volume_ratio,
                                  uint32_t sample_rate, uint8_t channels) {
  if (!pcm || samples == 0) {
    log_error("[Audio] pcm16 empty");
    return false;
  }
  if (channels != 1 && channels != 2) {
    log_error("[Audio] pcm16 bad channels=%u", (unsigned)channels);
    return false;
  }
  if (sample_rate == 0) {
    log_error("[Audio] pcm16 bad sample_rate=0");
    return false;
  }

  i2s_set_clk(I2S_NUM_1, sample_rate, I2S_BITS_PER_SAMPLE_16BIT, channels == 2 ? I2S_CHANNEL_STEREO : I2S_CHANNEL_MONO);
  play(pcm, samples, volume_ratio);
    ...
    switch (job.kind) {
      ...
      case AudioPlayJob::Kind::kStreamPcm16Chunk: {
        ...
        if (!s_stream_pcm_active) {
          ...
          play(job.pcm_heap, job.pcm_samples, s_stream_vol);
          if (job.free_mode == AudioHeapFree::kMalloc && job.pcm_heap) {
            ::free(job.pcm_heap);
          } else if (job.free_mode == AudioHeapFree::kHeapCaps && job.pcm_heap) {
            heap_caps_free(job.pcm_heap);
...
bool audio_stream_pcm16_push_copy(const int16_t* samples, size_t num_samples, uint32_t alloc_caps,
                                  float volume_ratio) {
  if (!samples || num_samples == 0) {
    return false;
  }
  ...
  if (alloc_caps == 0) {
    ...
    buf = (int16_t*)heap_caps_malloc(bytes, alloc_caps);
    free_caps = alloc_caps;
  }
  if (!buf) {
    return false;
  }
  memcpy(buf, samples, bytes);
  if (!audio_stream_pcm16_push_owned(buf, num_samples, free_caps, volume_ratio)) {
    if (free_caps == 0) {
      ::free(buf);
    } else {
      heap_caps_free(buf);
