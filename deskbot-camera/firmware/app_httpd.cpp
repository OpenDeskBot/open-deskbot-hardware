...
static void log_face_points(std::list<dl::detect::result_t> *results, int face_id)
{
    ...
    Serial.printf("FACE_STATUS: DETECTED count=%u face_id=%d\r\n", (unsigned)results->size(), face_id);
    int idx = 0;
    for (std::list<dl::detect::result_t>::iterator prediction = results->begin(); prediction != results->end(); ++prediction, ++idx)
    {
#if TWO_STAGE
        if (prediction->keypoint.size() >= 10)
        {
            Serial.printf(
                "FACE_KP[%d]: LE(%d,%d) ML(%d,%d) NOSE(%d,%d) RE(%d,%d) MR(%d,%d)",
                idx,
                (int)prediction->keypoint[0], (int)prediction->keypoint[1],
                (int)prediction->keypoint[2], (int)prediction->keypoint[3],
                (int)prediction->keypoint[4], (int)prediction->keypoint[5],
                (int)prediction->keypoint[6], (int)prediction->keypoint[7],
                (int)prediction->keypoint[8], (int)prediction->keypoint[9]);
            Serial.print("\r\n");
        }
        else
        {
            Serial.printf("FACE_KP[%d]: keypoint_count=%u (insufficient)\r\n", idx, (unsigned)prediction->keypoint.size());
        }
#else
        Serial.printf("FACE_BOX[%d]: x1=%d y1=%d x2=%d y2=%d\r\n", idx, (int)prediction->box[0], (int)prediction->box[1], (int)prediction->box[2], (int)prediction->box[3]);
#endif
    }
}

static void try_send_face_pos_ws(std::list<dl::detect::result_t> *results)
{
#if TWO_STAGE
    if (!results || results->empty()) {
        return;
    }
    const dl::detect::result_t &f = results->front();
    ...
    face_pos_ws_send_landmarks(&f.keypoint[0], c);
#endif
}
...
static esp_err_t capture_handler(httpd_req_t *req)
{
    ...
    if (fb->format == PIXFORMAT_RGB565
     ...
     && !recognition_enabled
        ...
        log_face_points(&results, face_id);
        try_send_face_pos_ws(&results);
