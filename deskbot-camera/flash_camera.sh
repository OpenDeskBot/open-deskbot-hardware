#!/usr/bin/env bash
# Deskbot Camera 固件工具：build | upload | log | all
# 在 deskbot-camera/ 目录下运行
set -euo pipefail

MODULE_ROOT="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$MODULE_ROOT/.." && pwd)"
FW_DIR="${OPEN_DESK_CAMERA_FW_DIR:-$MODULE_ROOT}"
PIO_ENV="${OPEN_DESK_CAMERA_PIO_ENV:-seeed_xiao_esp32s3}"
LOG_FILE="${OPEN_DESK_CAMERA_LOG:-$MODULE_ROOT/app.log}"
...
usage_copy_lines() {
  ...
  cat <<EOF
./flash_camera.sh build
./flash_camera.sh upload ${port}
./flash_camera.sh log ${port}
./flash_camera.sh all ${port}
EOF
  if [[ ${#SERIAL_PORTS[@]} -gt 1 ]]; then
    local p
    for p in "${SERIAL_PORTS[@]:1}"; do
      echo "./flash_camera.sh all ${p}"
    done
  fi
}
...
usage() {
  ...
  port="$(first_serial_port)"
  cat <<EOF
用法: ./flash_camera.sh build | upload | log | all [串口]
...
环境变量（可选）:
  SERIAL_PORT=/dev/ttyACM0 ./flash_camera.sh all
...
prepare_flash_env() {
  ...
  prepare_deskbot_flash_config "$FW_DIR" "$PIO_ENV" "camera"
}
